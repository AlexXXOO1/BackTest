...
STRATEGY_NAME = "pullback_strength_zone_select_strategy_v1"
...
def _ensure_volume_ratio_ma5(out: pd.DataFrame) -> pd.DataFrame:
    out = out.copy()

    if "volume_ratio_ma5" in out.columns:
        out["volume_ratio_ma5"] = pd.to_numeric(
            out["volume_ratio_ma5"],
            errors="coerce",
        ).replace([np.inf, -np.inf], np.nan)
        return out

    if "volume" not in out.columns:
        out["volume_ratio_ma5"] = np.nan
        return out
    ...
    out["volume_ratio_ma5"] = _safe_div(out["volume"], volume_ma5)
    out["volume_ratio_ma5"] = out["volume_ratio_ma5"].replace(
        [np.inf, -np.inf],
        np.nan,
    )
...
def add_pullback_strength_zone_features(
    ...
    out[short_pos_col] = (
        _safe_div(
            close - out[short_llv_col],
            out[short_hhv_col] - out[short_llv_col],
        )
        * 100.0
    )
    ...
    out[long_pos_col] = (
        _safe_div(
            close - out[long_llv_col],
            out[long_hhv_col] - out[long_llv_col],
        )
        * 100.0
    )
    ...
    out = _ensure_volume_ratio_ma5(out)
...
def select(
    ...
    min_long_pos_21: float = 65.0,
    min_volume_ratio_ma5: float = 1.05,
    **kwargs,
) -> pd.DataFrame:
    ...
    volume_ratio_ma5 = pd.to_numeric(out["volume_ratio_ma5"], errors="coerce")
    ...
    long_pos_filter = (
        long_pos >= float(min_long_pos_21)
    ).fillna(False)

    volume_filter = (
        volume_ratio_ma5 >= float(min_volume_ratio_ma5)
    ).fillna(False)
    ...
    out["v1_base_signal"] = base_mask.astype(bool)
    out["v1_long_pos_21_filter"] = long_pos_filter.astype(bool)
    out["v1_volume_ratio_ma5_filter"] = volume_filter.astype(bool)
    ...
    out["score_rank_key"] = (
        long_pos * 1.0
        + volume_ratio_ma5.fillna(0.0) * 10.0
        - short_pos * 0.5
