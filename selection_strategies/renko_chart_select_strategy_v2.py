...
selected = hard_brick_turn_strong
           AND (T0 J < 14 OR T-1 J > T-2 J)
           AND small_price_rise
           AND long_red_brick
"""
...
REQUIRED_INDICATOR_COLUMNS: set[str] = {
    ...
    "current_red_height", "previous_green_height", "J", "daily_return_pct",
}
...
DEFAULT_MAX_DAILY_RETURN_PCT = 3.0
...
def add_strategy_conditions(
    ...
    *,
    ...
    out["small_price_rise"] = (pd.to_numeric(out["daily_return_pct"], errors="coerce") <= max_daily_return_pct).fillna(False)
    out["long_red_brick"] = (current_red_height >= min_current_red_height).fillna(False)
    out["small_rise_long_red_brick_v2"] = out["small_price_rise"].astype(bool) & out["long_red_brick"].astype(bool)
    return out
