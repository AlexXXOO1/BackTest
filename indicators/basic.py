...
def add_kline_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["amplitude_pct"] = (_safe_div(high, low) - 1.0) * 100.0

    out["body_pct"] = (_safe_div(close - open_, open_)) * 100.0
    out["body_abs_pct"] = out["body_pct"].abs()

    # Shadows as percentage of close/open reference. These are generic facts,
    # not pattern decisions.
    max_oc = pd.concat([open_, close], axis=1).max(axis=1)
    min_oc = pd.concat([open_, close], axis=1).min(axis=1)

    out["upper_shadow_pct"] = _safe_div(high - max_oc, close) * 100.0
    out["lower_shadow_pct"] = _safe_div(min_oc - low, close) * 100.0
