...
def add_v8_risk_indicators(
    ...
    """
    Add v8 hard-filter risk indicators.
    ...
    - v8_hard_filter_pass is false when any of the four v8 hard-filter risks is true.
    """
    df = df.copy()
    ...
    df["v8_hard_filter_pass"] = ~(
        df["prior_20d_accelerated_huge_volume_bear"]
        | df["prior_20d_shrink_limit_up"]
        | df["long_lower_shadow_hammer"]
        | df["limit_up_red_brick"]
    )
    return df
