...
"""
B1 stage-low selection strategy v0.
...
B1 selection rule:
    kdj_j < j_threshold only.

Other B1 condition checks are retained as factor columns in the pool parquet
for later bucket analysis and validation. T-1 J is retained as a continuous numeric
factor, not as a binary threshold flag. The former extreme-volume flag is not exported.
"""
...
STRATEGY_NAME = "b1_stage_low_select_strategy_v0"
...
def select(
    ...
    **kwargs,
    ...
    """Return B1 rows selected only by low KDJ J; keep other B1 checks as factors."""
