...
def _add_executable_entry_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "t1_open" in out.columns and "close" in out.columns and "t1_open_gap_pct" not in out.columns:
...
def _calc_win_count(target: pd.Series) -> int:
    target = _safe_numeric(target).dropna()
    return int((target > 0).sum())


def _calc_loss_count(target: pd.Series) -> int:
    target = _safe_numeric(target).dropna()
    return int((target <= 0).sum())
...
def _classify_binary_pattern(
    false_mean_return: float,
    true_mean_return: float,
    false_up_ratio: float,
    true_up_ratio: float,
) -> tuple[str, str, str, str]:
    true_minus_false_return = true_mean_return - false_mean_return
    true_minus_false_up_ratio = true_up_ratio - false_up_ratio

    true_return_better = true_minus_false_return > RETURN_EPS
    true_up_better = true_minus_false_up_ratio > UP_RATIO_EPS
    false_return_better = true_minus_false_return < -RETURN_EPS
    false_up_better = true_minus_false_up_ratio < -UP_RATIO_EPS

    if (true_return_better and not false_up_better) or (true_up_better and not false_return_better):
        return (
            "binary_true_better",
            "prefer_true",
            "false_side",
            "The true/1 group has better forward-return or win-rate evidence than the false/0 group.",
        )

    if (false_return_better and not true_up_better) or (false_up_better and not true_return_better):
        return (
            "binary_false_better",
            "prefer_false",
            "true_side",
            "The false/0 group has better forward-return or win-rate evidence than the true/1 group.",
        )

    return (
        "binary_no_clear_edge",
        "neutral",
        "none",
        "The true/1 and false/0 groups do not show a clear edge difference.",
    )
...
def analyze_indicator_direction(
    ...
    df = _add_executable_entry_features(df.copy(deep=False))  # avoid full pool deep copy
    ...
    for factor in factors:
        ...
        tmp = work.loc[:, tmp_cols].copy(deep=False)
        ...
        is_binary_factor = unique_count == 2
        ...
        bucket = (
            ...
            .agg(
                ...
                win_count=(target_col, _calc_win_count),
                loss_count=(target_col, _calc_loss_count),
            )
            .sort_values("bucket")
        ...
        if is_binary_factor:
            bucket_pattern, action_hint, risk_side, pattern_reason = _classify_binary_pattern(
                false_mean_return=bottom_mean_return,
                true_mean_return=top_mean_return,
                false_up_ratio=bottom_up_ratio,
                true_up_ratio=top_up_ratio,
            )
        else:
        ...
        if export_member_detail:
            member_detail = tmp.copy(deep=False)
    ...
    if not summary.empty:
        pattern_order = {
            ...
            "higher_better": 3,
            "lower_better": 4,
            "binary_true_better": 5,
            "binary_false_better": 6,
            "binary_no_clear_edge": 7,
            "weak_or_unclear": 8,
        }
    ...
    if member_rows:
        ...
        if sort_cols and len(bucket_member_detail) <= 300_000:
            ascending = [True] * len(sort_cols)
            if "score_rank_key" in sort_cols:
                ascending[sort_cols.index("score_rank_key")] = False
            bucket_member_detail = bucket_member_detail.sort_values(sort_cols, ascending=ascending, kind="stable").reset_index(drop=True)
    else:
        bucket_member_detail = pd.DataFrame()
...
def main() -> None:
    ...
    if not args.include_unselected and "selected" in df.columns:
        ...
        if selected_num.notna().any():
            df = df.loc[selected_num.fillna(0).astype(int) == 1].copy(deep=False)
        else:
            df = df.loc[df["selected"].astype(bool)].copy(deep=False)
    ...
    summary = result.summary.copy(deep=False)
    bucket_detail = result.bucket_detail.copy(deep=False)
    bucket_member_detail = result.bucket_member_detail.copy(deep=False)
